...
INSERT INTO public.emails (email_key, email_from, email_to, email_cc, email_bcc, email_subject, email_html_body, email_text_body, email_state, created_at, processed_at)
    VALUES ('12345678-0001-0001-0001-000000000011'::uuid, 'no-reply@vetchi.org', ARRAY['admin@domain-onboarded.example'], NULL, NULL, 'Welcome to Vetchi Subject', 'Welcome to Vetchi HTML Body', 'Welcome to Vetchi Text Body', 'PROCESSED', timezone('UTC'::text, now()), timezone('UTC'::text, now()));
INSERT INTO public.employers (id, client_id_type, employer_state, onboard_admin_email, onboard_secret_token, token_valid_till, onboard_email_id, created_at)
    VALUES ('12345678-0001-0001-0001-000000000201'::uuid, 'DOMAIN', 'ONBOARDED', 'admin@domain-onboarded.example', 'blah', timezone('UTC'::text, now()) + interval '1 day', '12345678-0001-0001-0001-000000000011'::uuid, timezone('UTC'::text, now()));
...
INSERT INTO public.org_users (id, email, password_hash, org_user_roles, org_user_state, employer_id, created_at)
    VALUES ('12345678-0001-0001-0001-000000040001'::uuid, 'admin@domain-onboarded.example', '$2a$10$p7Z/hRlt3ZZiz1IbPSJUiOualKbokFExYiWWazpQvfv660LqskAUK', ARRAY['ADMIN']::org_user_roles[], 'ACTIVE_ORG_USER', '12345678-0001-0001-0001-000000000201'::uuid, timezone('UTC'::text, now()));
COMMIT;

-- Cleanup data for 0034-incognito-posts_test.go
...
WHERE comment_id IN (
    ...
    WHERE ip.author_id LIKE '12345678-0034-%'
);

-- Delete incognito post votes
DELETE FROM incognito_post_votes 
WHERE incognito_post_id IN (
    SELECT id FROM incognito_posts WHERE author_id LIKE '12345678-0034-%'
);
...
DELETE FROM incognito_posts WHERE author_id LIKE '12345678-0034-%';

-- Delete hub users created for this test
DELETE FROM hub_users WHERE id LIKE '12345678-0034-%';

DELETE FROM tags 
WHERE id IN ('technology', 'careers', 'personal-development', 'mentorship', 'startups'); 
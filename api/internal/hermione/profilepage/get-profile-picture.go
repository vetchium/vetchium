...
func GetProfilePicture(h wand.Wand) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		h.Dbg("Entered GetProfilePicture")

		// Verify that the request comes from an authenticated user
		_, ok := r.Context().Value(middleware.HubUserCtxKey).(db.HubUserTO)
		if !ok {
		...
		if len(parts) != 4 { // /hub/profile-picture/{handle}
			h.Dbg("invalid URL path", "path", r.URL.Path)
			http.Error(w, "", http.StatusBadRequest)
			return
		}
		requestedHandle := parts[3]
